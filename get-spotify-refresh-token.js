...
// Log what we loaded (masked) so we can see if Fly / local are actually getting values
console.log("[Spotify] Loaded credentials:", {
  clientIdPrefix: SPOTIFY_CLIENT_ID ? SPOTIFY_CLIENT_ID.slice(0, 6) : null,
  clientIdLength: SPOTIFY_CLIENT_ID ? SPOTIFY_CLIENT_ID.length : 0,
  secretLength: SPOTIFY_CLIENT_SECRET ? SPOTIFY_CLIENT_SECRET.length : 0,
...
async function getAccessToken() {
  if (!SPOTIFY_CLIENT_ID || !SPOTIFY_CLIENT_SECRET || !SPOTIFY_REFRESH_TOKEN) {
    throw new Error(
      "Spotify credentials missing. Check SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_REFRESH_TOKEN"
    );
  }
  ...
  // Extra debug so we can see the scopes
  if (data.scope) {
